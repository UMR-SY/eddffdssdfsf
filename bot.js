...
client.on('message', async msg => {
   ...
   .setTitle(`:loud_sound: Şuanki Ses Seviyesi: **${serverQueue.volume}**`)
    ...
    .setColor('RANDOM'));
		  return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setColor('RANDOM')
     .setTitle('TheOrder Müzik | Şarkı Kuyruğu')
    ...
    .addField('Şu anda çalınan: ' + `${serverQueue.songs[0].title}`);
      ...
      .setColor('RANDOM'));
		...
		if (serverQueue && !serverQueue.playing) {
			...
			return msg.channel.sendEmbed(new Discord.RichEmbed()
      .setTitle("**<a:ALTIGENTAS:602376872055275521> Müzik Senin İçin Devam Etmekte!**")
      .setColor('RANDOM'));
		}
		return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setTitle("** Çalan Müzik Bulunmamakta.**")
...
async function handleVideo(video, msg, voiceChannel, playlist = false) {
	...
	if (!serverQueue) {
		...
		try {
			...
			console.error(`<a:XXX:588397643017879583> **Şarkı Sisteminde Problem Var Hata Nedeni: ${error}**`);
			queue.delete(msg.guild.id);
			return msg.channel.sendEmbed(new Discord.RichEmbed()
      .setTitle(`<a:XXX:588397643017879583> **Şarkı Sisteminde Problem Var Hata Nedeni: ${error}**`)
      .setColor('RANDOM'))
		}
	} else {
...
function play(guild, song) {
	...
	const dispatcher = serverQueue.connection.playStream(ytdl(song.url))
		.on('end', reason => {
			if (reason === '<a:XXX:588397643017879583> | **Yayın Akış Hızı Yeterli Değil.**') console.log('Müzik Bitti.');
...
client.on('guildMemberAdd', (member) => {
    if (member.guild.id == "592024867810050051") {
        ...
        client.channels.get("594783627297030177").send(`<@${member.id}> adlı kişi sunucumuza katıldı, seni karşılamaktan şeref duyarız :)`)
        return
    }
});
...
client.on('guildMemberRemove', (member) => {
    if (member.guild.id == "592024867810050051") {
        console.log(`${member.user.username} adlı kişi destek sunucumuzdan ayrıldı, sağlık olsun.`)
        client.channels.get("594783627297030177").send(`${member.user.username} adlı kullanıcı sunucudan ayrıldı, tam bir dram!`)
        return
    }
});
...
.setColor("GREEN")
  ...
  try {
    let giriscikiskanalID = giriscikis[member.guild.id].kanal;
    let giriscikiskanali = client.guilds.get(member.guild.id).channels.get(giriscikiskanalID);
    giriscikiskanali.send(`:loudspeaker:<a:onaylandI:586709716101496862> Hoşgeldin **${member.user.tag}** Rolün Başarıyla Verildi.`);
  } catch (e) { // eğer hata olursa bu hatayı öğrenmek için hatayı konsola gönderelim.
    return console.log(e)
  }
...
client.on('message', msg => {
  ...
  return msg.guild.me.setNickname(`${random.emoji}` + `${client.user.username}`)
});
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
.then(channel =>
 channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|Bot Kanalları|▬▬|")));
...
.then(channel =>
            ...
            .then(channel => channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|Bot Kanalları|▬▬|")));
...
message.guild.createChannel(`🎮》Lol Odası`, 'voice')
.then(channel =>
 channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|Oyun Odaları|▬▬|")))
 message.guild.createChannel(`🎮》Zula Odası`, 'voice')
 .then(channel =>
  channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|Oyun Odaları|▬▬|")))
 message.guild.createChannel(`🎮》Counter Strike Odası`, 'voice')
.then(channel =>
 channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|Oyun Odaları|▬▬|")))
 message.guild.createChannel(`🎮》Pubg Odası`, 'voice')
 .then(channel =>
  channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|Oyun Odaları|▬▬|")))
  message.guild.createChannel(`🎮》Fortnite Odası`, 'voice')
  .then(channel =>
   channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|Oyun Odaları|▬▬|")))
   message.guild.createChannel(`🎮》MineCraft Odası`, 'voice')
   .then(channel =>
    channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|Oyun Odaları|▬▬|")))
    message.guild.createChannel(`🎮》RobLox Odası`, 'voice')
    .then(channel =>
     channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|Oyun Odaları|▬▬|")))
     message.guild.createChannel(`🎮》WolfTeam Odası`, 'voice')
     .then(channel =>
      channel.setParent(message.guild.channels.find(channel => channel.name === "|▬▬|Oyun Odaları|▬▬|")))
...
;  kanal31.send(girismesaj ? girismesaj.replace('-kullanıcı-', `\`${member.user.tag}\``) .replace('-sunucu-', `${member.guild.name}`) : `<a:parti:606925584966090967> \`${member.user.tag}\` adlı kullanıcı sunucuya katıldı.`);
});
...
client.on("guildMemberRemove", async member => {
  ...
  kanal31.send(cikismesaj ? cikismesaj.replace('-kullanıcı-', `\`${member.user.tag}\``) .replace('-sunucu-', `${member.guild.name}`) : `<a:parti:606925584966090967> \`${member.user.tag}\` adlı kullanıcı sunucudan ayrıldı.`);
});
